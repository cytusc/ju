...
class jiuAction extends FirstendAction
{
    ...
    public function index()
    {
        ...
        if (false === $jiu = F('jiu')) {
            $jiu = D('items_cate')->cate_cache();
        }
        $this->assign('cate_list', $jiu); // 分类
        ...
        if (C('yh_site_cache')) {
            if (false === $items = S($file)) {
                ...
                foreach ($items_list as $key => $val) {
                    ...
                    $items['item_list'][$key]['cate_name'] = $jiu['p'][$val['cate_id']]['name'];
