...
class indexAction extends FirstendAction {
	...
	//			break;
		...
		/*if(C('yh_index_cids')){
			$where['cate_id'] =  array('in',C('yh_index_cids'));
		}*/
...
//	    }
	...
	//	}
		...
		$map['isq'] = 1;
		$map['Quan_id'] = array('neq','');
		$map['last_time'] = array('lt',strtotime(date('Y-m-d H:i:s',strtotime('-2 hours'))));
		$map['coupon_end_time'] = array('gt',strtotime(date('Y-m-d H:i:s')));
		$orlike = $this->_mod->where($map)->limit('0,30')->order('id desc')->getField('num_iid,Quan_id'); //select();
		foreach ($orlike as $key => $val) {
			$quanarr = $quanarr."'".$key.'-'.$val."',";
		}
		$quanarr=rtrim($quanarr,",");
		$quanarr="var itemarr=[$quanarr];";
		$this->assign('quanarr',$quanarr);
		$ad = $this->_ad->where(array('status'=>'1'))->order('id desc')->select();
		$this->assign('ad_list', $ad);
		$pager = $this->_pager($count, $page_size);
		$this->assign('page', $pager->wshow());
...
//		}
  ...
  /**
	...
	//	if (false === $cate_list = S('cate_list')) {
		...
		$map['coupon_end_time'] = array('gt',strtotime(date('Y-m-d H:i:s')));
		$orlike = $this->_mod->where($map)->limit('0,30')->order('id desc')->getField('num_iid,Quan_id'); //select();
		if(!$orlike){unset($map['cate_id']);$orlike = $this->_mod->where($map)->limit('0,30')->order('id desc')->getField('num_iid,Quan_id');}
		foreach ($orlike as $key => $val) {
			$quanarr = $quanarr."'".$key.'-'.$val."',";
		}
		$quanarr=rtrim($quanarr,",");
		$quanarr="var itemarr=[$quanarr];";
		$this->assign('quanarr',$quanarr);
		$Number = $this->_mod->where(array('isq'=>'1','cate_id'=>$cid))->count();
		$this->assign('Number', $Number);
if(C('URL_MODEL')==2 && C('APP_SUB_DOMAIN_DEPLOY')==true){
	  $pager = $this->_pager($count, $page_size, '/cate');
}else{
$pager = $this->_pager($count, $page_size, '/m/index/cate');
}
		$this->assign('page', $pager->wshow());
		$this->assign('zpage', $pager->zshow());
		$this->assign('total_item',$count);
		$this->assign('ajaxurl',U('index/cate',array('cid'=>$cid,'p'=>$index_info['p'],'sort'=>$index_info['sort'])));
		$this->assign('cate_name',$cinfo['name']);
        $this->assign('nav_curr', 'index');
