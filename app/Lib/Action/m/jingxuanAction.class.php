...
class jingxuanAction extends FirstendAction {
    ...
    public function index() { 
		...
		if(false === $paoliang = F('paoliang')) {
			$paoliang = D('items_cate')->cate_cache();
		}
		$this->assign('cate_list', $paoliang['p']); //分类
		...
		if(C('yh_site_cache')){
			if(false === $items = S($file)){
				...
				foreach($items_list as $key=>$val){
					...
					$items['item_list'][$key]['cate_name']		=$paoliang['p'][$val['cate_id']]['name']; 
		...
		if(!$orlike){unset($map['quan_pl']);$orlike = $this->_mod->where($map)->limit('0,30')->order('id desc')->getField('num_iid,Quan_id');}
