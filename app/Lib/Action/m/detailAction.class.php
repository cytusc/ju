...
class detailAction extends FirstendAction {
    ...
    public function index() {
        $id = I('id', '','trim');
        $iid = D('items')->cache(true, 10 * 60)->where(array('id'=>$id))->getField('num_iid');
		$iid = $iid+1;
		$this->assign('iid', $iid);
		...
		$txurl = C('yh_site_url').U('detail/index',array('id'=>$item['id']));
		...
		if(C('yh_site_cache')){
			...
			if(false === $orlike = S($file)){
				if($cid){
					$orlike = $this->_mod->where(array('cate_id'=>$cid))->limit('0,12')->order('id desc')->select();
				}else{
		...
		foreach($orlike as $key=>$val){
			...
			if($val['coupon_start_time']>time()){
				$items[$key]['click_url']	=U('detail/index',array('id'=>$val['id']));
			...
			$url = C('yh_site_url').U('detail/index',array('id'=>$val['id']));
		...
		if(strlen($item[Quan_id])>20 and $item[isq]==1 and $item[last_time]<=strtotime(date('Y-m-d H:i:s',strtotime('-1 hours')))){$quanarr = "'".$item[num_iid].'-'.$item[Quan_id]."',";}
		foreach ($orlike as $key => $val) {
		...
		$item['price'] = floattostr($item['price']);
		$item['coupon_price'] = floattostr($item['coupon_price']);
