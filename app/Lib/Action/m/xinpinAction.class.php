...
class xinpinAction extends FirstendAction {
    ...
    public function index() { 
		...
		$tomorr_wh['coupon_start_time'] = array(array('egt',$today_str),array('elt',$tomorr_str)) ;
		...
		$this->assign('cate_list', $xinpin['p']); //分类
		...
		$map['coupon_start_time'] = array(array('gt',strtotime(date('Y-m-d'))),array('lt',strtotime(date('Y-m-d',strtotime('+1 day')))));
		...
		if(!$orlike){unset($map['coupon_start_time']);$orlike = $this->_mod->where($map)->limit('0,30')->order('id desc')->getField('num_iid,Quan_id');}
