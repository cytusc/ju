...
class ershiAction extends FirstendAction {
    ...
    public function index() { 
		...
		if(false === $ershi = F('ershi')) {
			$ershi = D('items_cate')->cache(true, 10 * 60)->cate_cache();
		}
		$this->assign('cate_list', $ershi['p']); //分类
		...
		$map['coupon_price'] = array(array('gt',10),array('lt',20));
		$map['last_time'] = array('lt',strtotime(date('Y-m-d H:i:s',strtotime('-1 hours'))));
		$map['coupon_end_time'] = array('gt',strtotime(date('Y-m-d H:i:s')));
		$orlike = $this->_mod->where($map)->limit('0,20')->order('id desc')->getField('num_iid,Quan_id'); //select();
		if(!$orlike){unset($map['coupon_price']);$orlike = $this->_mod->where($map)->limit('0,20')->order('id desc')->getField('num_iid,Quan_id');}
