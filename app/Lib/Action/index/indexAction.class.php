...
class indexAction extends FirstendAction
{
    ...
    public function index()
    {
        ...
        $today_wh['pass'] = '1';
        $today_wh['isshow'] = '1';
        $article_list = M('article')->where('status=1 and cate_id=1')
        ...
        if (C('yh_index_not_text')) {
            $not_arr = explode(",", C('yh_index_not_text'));
            $arrs = array();
        ...
        if (C('yh_wait_time') == '1') {
            $where['coupon_start_time'] = array(
                'egt',
                time()
            );
        } elseif (C('yh_wait_time') == '2') {
            $where['coupon_start_time'] = array(
                'elt',
                time()
            );
      ...
      //  if (NULL === $cate_list = F('cate_list')) {
       ...
       // }
        ...
        $cate_data = $this->_cate_mod->where(array(
            'status' => '1'
        ))->count();
        ...
        $pager = $this->_pager($count, $page_size, '/index');
        $this->assign('page', $pager->kshow());
        $this->assign('zpage', $pager->zshow());
        $this->assign('total_item', $count);
	    //$waitcount = $this->_mod->where('tuisong=0 and pass=1')->count();
    ...
    public function so()
    {
         ...
         * 文章列表
         */
        $article_list = M('article')->cache(true, 10 * 60)->where('status=1 and cate_id=1')
            ->order('ordid asc,id desc')
            ->limit(6)
            ->select();
        $this->assign('article_list', $article_list); // 文章列表
      ...
      //  $today_wh['pass'] = '1';
       ...
       // $where['pass'] = '1';
        ...
        foreach ($items_list as $key => $val) {
            ...
            if ($items[$key][last_time] < strtotime(date('Y-m-d H:i:s', strtotime('-2 hours'))) and $quanarriii <= 50 and strlen($items[$key][Quan_id]) > 20) {
                $quanarr = $quanarr . "'" . $items[$key][num_iid] . '-' . $items[$key][Quan_id] . "',";
                $quanarriii ++;
            }
            $pagecount ++;
            if ($val['sellerId']) {
        ...
        $this->assign('nav_curr', 'index');
        $page_seo = array(
            'title' => '搜索"' . $k . '"的宝贝结果页 - ' . C('yh_site_name')
        );
    ...
    public function cate()
    {
        ...
        if (C('yh_site_cache')) {
            ...
            if (false === $cinfo = S($file)) {
                $cinfo = $this->_cate_mod->where(array(
                    'id' => $cid
                ))->find();
                S($file, $cinfo);
            }
        ...
        if ($cinfo['shop_type']) {
            $map['shop_type'] = $cinfo['shop_type'];
        }
        if ($cinfo['mix_price'] > 0) {
            $map['coupon_price'] = array(
                'egt',
                $cinfo['mix_price']
            );
        }
        if ($cinfo['max_price'] > 0) {
            $map['coupon_price'] = array(
                'elt',
                $cinfo['max_price']
            );
        }
        if ($cinfo['max_price'] > 0 && $cinfo['mix_price'] > 0) {
            $map['coupon_price'] = array(
                array(
                    'egt',
                    $cinfo['mix_price']
        ...
        if ($cinfo['mix_volume'] > 0) {
            $map['volume'] = array(
                'egt',
                $cinfo['mix_volume']
            );
        }
        if ($cinfo['max_volume'] > 0) {
            $map['volume'] = array(
                'elt',
                $cinfo['max_volume']
            );
        }
        if ($cinfo['max_volume'] > 0 && $cinfo['mix_volume'] > 0) {
            $map['volume'] = array(
                array(
                    'egt',
                    $cinfo['mix_volume']
...
//		$map['tuisong'] = '1';
        ...
        if (C('yh_site_cache')) {
            ...
            if (false === $subnav = S($file)) {
                $subnav = $this->_cate_mod->where(array(
                    'pid' => $cid
                ))->select();
                if ($cinfo['pid'] && ! $subnav) {
                    $subnav = $this->_cate_mod->where(array(
        ...
        if (C('yh_site_cache')) {
            ...
            if (false === $items = S($file)) {
                ...
                $items_list = $this->_mod->where($map)
                    ->order($order)
                    ->limit($start . ',' . $page_size)
                    ->select();
                $items = array();
        ...
        $quanarr = "var arr=[$quanarr];";
        $this->assign('quanarr', $quanarr);
        $pager = $this->_pager($count, $page_size, '/cate');
        $this->assign('page', $pager->kshow());
        $this->assign('zpage', $pager->zshow());
        $this->assign('total_item', $count);
		$waitcount = $this->_mod->where('tuisong=0 and pass=1')->count();
		$this->assign('waitcount', $waitcount);
        $this->assign('ajaxurl', U('index/cate', array(
            'cid' => $cid,
            'p' => $index_info['p'],
            'sort' => $index_info['sort']
        )));
