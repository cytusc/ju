...
class indexAction extends BackendAction {
 ...
 //不要删除域名否则无法获得系统更新提醒       
        $version = json_decode(file_get_contents('http://www.tuiquanke.com/api/cms_version'), true);
        if($version['result']['version'] == FTX_VERSION){
            $system_info['yhxia_version'] = $version['result']['name'];
        }
        else{
            $system_info['yhxia_version'] = '<a href="'.$version['result']['url'].'" target="_blank">'.$version['result']['name'].'</a>';
        }
